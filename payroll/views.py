...
from reportlab.platypus import Image, Paragraph, Spacer,Frame, Flowable
...
class PayslipPDFView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        header_table.setStyle(TableStyle([
            ...
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),  # Horizontally align the logo and header text
        ]))

        payslip_content.extend([header_table, Spacer(20, 20)])

        # Line below the header
        payslip_content.append(Paragraph("-----------------------------------------------------------------------------------------------------------------------",styles['Normal']))

        title_text = "Staff Payslip"
    ...
    def add_footer(self, canvas, doc):
            ...
            footer_table = Table([[Paragraph(footer_text, footer_style)]], colWidths=[doc.width - 2 * doc.leftMargin])
            ...
            footer_table.drawOn(canvas, doc.leftMargin, doc.bottomMargin - 15)

            # Line above the footer
            line_width = doc.width * doc.leftMargin-10
            line_height = doc.bottomMargin - 20
            underline_line = Underline(450, line_height)
            underline_line.wrapOn(canvas, line_width, 0)
            underline_line.drawOn(canvas, doc.leftMargin, line_height)
